...
LIBSAVED="gdk-pixbuf gstr libv4lconvert.so libGLdispatch.so libgpm.so libGLU.so libglut.so libslang.so libsoup libtheora.so svg $lib_audio_keywords $lib_browser_launcher"
...
_remove_more_bloatwares() {
	...
	lib_remove="gcc gconv libaom libavcodec libPy libx26 python"
